...
public:
    vector<News *> getRecords()
    {
        ...
        string url = "https://newsapi.org/v2/everything?q=election&from=2023-08-18&sortBy=popularity&apiKey=b0d823284f374733b520c8b0d6e779b6";
...
{
    Newsapi newsapi;
    NewsSeeker newsSeeker;

    vector<News *> allrecords = newsapi.getRecords();
    for (News *news : allrecords)
    {
        newsSeeker.agregar(news->getAuthor(), news->getTitle(), news->getDescription());
    }
    cout << endl;
    newsSeeker.buscar("Trump");
    newsSeeker.reordenar(8, -90);
    newsSeeker.buscar("Trump");
    // newsSeeker.cantNoticias();
    return 0;
}